...
gulp.task('dist', ['config', 'vendors', 'assets', 'i18n', 'fonts-dist', 'styles-dist', 'scripts-dist'], function () {
  return gulp.src(['./src/app/index.html'])
    ...
    .pipe(g.inject(gulp.src('../public/styles/' + bower.name + '-' + bower.version + '.min.css'), {addRootSlash: false, ignorePath: 'vagrant/MakeMyBooking/public'}))
...
function dist (ext, name, opt) {
  ...
  return lazypipe()
    ...
    .pipe(opt.ngmin ? gulp.dest : noop, '../public')
